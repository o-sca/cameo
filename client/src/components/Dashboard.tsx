import React, { useState, useEffect } from 'react';
import styled from '@emotion/styled';
import '../index.css';


import {
  Table,
  TableCaption,
  TableContainer,
  Tbody,
  Tfoot,
  Th,
  Thead,
  Tr,
  Td,
  Button,
  Input
} from '@chakra-ui/react';

const Stack = styled(Button)`
  color: red;
`;

const StyledTableContainer = styled(TableContainer)`

`;

const StyledTable = styled(Table)`
  border-collapse: separate;
  border-spacing: 0px;
  overflow: hidden;
`;

const StyledTh = styled(Th)`
  background-color: white;
  color: #2640FF;
  font-weight: bold;
  border: 2px solid #dce1ff;
  padding: 12px 24px;
`;

const StyledTd = styled(Td)`
  border: 1px solid #ddd;
  padding: 8px;
`;

interface JobInformation {
  status: string;
  company_name: string;
  job_title: string;
  url: string;
  date_posted: string;
  date_applied: string;
}

const PlayerTable = () => {
  const [jobInformation, setJobInformation] = useState<JobInformation[]>([]);

  useEffect(() => {
    const user_id = localStorage.getItem('userId');
    if (user_id) {
      fetch('https://6af8-2001-569-70c7-4300-8095-1aac-6db8-57ad.ngrok.io/api/v1/alljobs', {
        method: 'POST',
        body: JSON.stringify({ userId: user_id }),
        headers: {
          'Content-Type': 'application/json',
        },
      })
        .then((response) => response.json())
        .then((data) => setJobInformation(data.data))
        .catch((error) => console.log(error));
    }
  }, []);

  console.log(jobInformation)
  return (
    <>
      <div className="body">
        <header>
          <div className="header-content">
            <img src="data:image/svg+xml;base64," alt="Cameo Logo" />
            <a href="#">Logout</a>
          </div>
        </header>
        <div className="container">
          <h1>My Cameo Sheet</h1>
          <div className="content">
            <div className="input-btns">
              <div className="search"><Input variant='outline' placeholder='🔎' width="180px" borderRadius="10px" borderColor="#DCE1FF" boxShadow="none" p={'14'}></Input>
              </div>
              <div className="buttons">
                <Button className="edit-btn">Edit</Button>
                <Button className="delete-btn">Delete</Button>
              </div>
            </div>
            <div className="table">
              <StyledTableContainer pt="25px" pb="15px" w="65vw">
                <StyledTable variant="simple" size="lg" alignItems={'center'}>
                  <TableCaption mt="10">Job Information</TableCaption>
                  <Thead>
                    <Tr>
                      <StyledTh textAlign={'center'}>Status &nbsp;⏳</StyledTh>
                      <StyledTh textAlign={'center'}>Company Name &nbsp;🏢</StyledTh>
                      <StyledTh textAlign={'center'}>Job Title &nbsp;🔍</StyledTh>
                      <StyledTh textAlign={'center'}>Date Posted &nbsp;🗓</StyledTh>
                    </Tr>
                  </Thead>
                  <Tbody textAlign={'center'}>
                    {jobInformation.map((job) => {
                      return (
                        <tr key={job.job_title}>
                          <StyledTd>{job.status}</StyledTd>
                          <StyledTd>{job.company_name}</StyledTd>
                          <StyledTd>{job.job_title}</StyledTd>
                          <StyledTd>{job.date_posted}</StyledTd>
                        </tr>
                      );
                    })}
                  </Tbody>
                  <Tfoot>
                    <Tr></Tr>
                  </Tfoot>
                </StyledTable>
              </StyledTableContainer>
            </div>
          </div>
          <div className="footer">
            <hr></hr>
            <p className="copyright">© 2023 Cameo Inc. All rights reserved.</p>
          </div>
        </div>
      </div>
    </>
  );
}

export default PlayerTable;
