...
interface Props {
  ...
  setJobInformation: React.Dispatch<React.SetStateAction<JobInformation[]>>;
}

const PlayerTable: React.FC<Props> = ({ jobInformation, setJobInformation }) => {
  const [status, setStatus] = useState<string[]>(['Status', 'Company Name', 'Job Title', 'Date Posted']);

  useEffect(() => {
    const user_id = localStorage.getItem('userId');
    if (user_id) {
      fetch('https://5d2e-2001-569-70c7-4300-8095-1aac-6db8-57ad.ngrok.io/api/v1/alljobs', {
        method: 'POST',
        ...
        .then((data) => setJobInformation(data))
    ...
    <body className="body">
      ...
      <h1>My Cameo Sheet</h1>
        <div className="content">
          ...
          <div className="table">
            <StyledTableContainer pt="25px" pb="15px" w="65vw">
          <StyledTable variant="simple" size="lg" alignItems={'center'}>
            <TableCaption mt="10">Job Information</TableCaption>
            <Thead>
              <Tr>
                <StyledTh textAlign={'center'}>Status &nbsp;⏳</StyledTh>
                <StyledTh textAlign={'center'}>Company Name &nbsp;🏢</StyledTh>
                <StyledTh textAlign={'center'}>Job Title &nbsp;🔍</StyledTh>
                <StyledTh textAlign={'center'}>Date Posted &nbsp;🗓</StyledTh>
              </Tr>
            </Thead>
            <Tbody textAlign={'center'}>
              {jobInformation.map((job) => {
                return (
                  <tr key={job.jobTitle}>
                    <StyledTd>{job.status}</StyledTd>
                    <StyledTd>{job.companyName}</StyledTd>
                    <StyledTd>{job.jobTitle}</StyledTd>
                    <StyledTd>{job.datePosted}</StyledTd>
                  </tr>
                );
              })}
            </Tbody>
        ...
        <div className="footer">
          <hr></hr>
          <p className="copyright">© 2023 Cameo Inc. All rights reserved.</p>
        </div>
      </div>
    </body>
  );
